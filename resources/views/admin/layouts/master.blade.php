    ...
    <script>
        window.addEventListener('load', function() {
            ...
            setTimeout(function() {
                ...
                // Loại bỏ lớp 'loading' khỏi body để bật lại cuộn
                document.body.classList.remove('loading');
            }, 500); // 500ms tương ứng với transition trong CSS
        });
    </script>
    <style>
        body.loading {
...
<body class="loading">
    ...
    <script>
        ...
        if (success != null) {
            show(success)
            // Xóa trạng thái sau khi hiển thị toast
            localStorage.removeItem('success');
        }

        function show(message, danger = false) {
            toastBody.textContent = message;
            liveToast.classList.remove('text-bg-danger', 'text-bg-success');
        ...
        function fetch_and_handle(url, method, data = null, is_form_data = false) {
            ...
            if (is_form_data) {
                ...
                options.body = JSON.stringify(data) // Gửi dữ liệu JSON cho phương thức POST, PUT, PATCH
            }

            return fetch(url, options)
                .then(res => {
                    if (!res.ok) {
                        // console.log('Lỗi mạng', res)
                        show('Đã có lỗi xảy ra, vui lòng thử lại', true)
                        return res.json().then(data => {
                            // Ném lỗi với đối tượng lỗi JSON
                            throw data.errors;
                        });
                    }
                    return res.json();
