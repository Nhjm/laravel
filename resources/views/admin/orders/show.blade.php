                ...
                <div class="card-body">
                    <div class="mb-3">
                        <div>
                            ...
                            <a target="_blank" href="{{ route('admin.orders.export_order', $order) }}" type="button"
                                class="btn btn-success add-btn"><i class="ri-file-list-fill align-bottom me-1"></i>In hóa
                                đơn</a>
                    ...
                    <table id="example" class="table table-bordered dt-responsive nowrap table-striped align-middle"
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td data-ordering="false">{{ $order->id }}</td>
                                <td data-ordering="false">{{ $order->user_name }}</td>
                                <td data-ordering="false">
                                    {{ $order->user_email }}
                                </td>
                                <td data-ordering="false">{{ $order->user_phone }}</td>
                                <td>{{ $order->user_address }}</td>
                                <td data-ordering="false">{{ $order->user_notes }}</td>
                                <td data-ordering="false">{{ number_format($order->total_price) }}</td>
                                <td data-ordering="false">
                                    <select data-id="{{ $order->id }}" class="form-select form-select-sm select_order"
                                        aria-label=".form-select-lg">
                                        @foreach ($status_order as $key => $name)
                                            <option value="{{ $key }}" @selected($order->status_order == $key)>
                                                {{ $name }}</option>
                                        @endforeach
                                    </select>
                                </td>
                                <td data-ordering="false">
                                    <select data-id="{{ $order->id }}" class="form-select form-select-sm select_payment"
                                        aria-label=".form-select-lg">
                                        @foreach ($status_payment as $key => $name)
                                            <option value="{{ $key }}" @selected($order->status_payment == $key)>
                                                {{ $name }}</option>
                                        @endforeach
                                    </select>
                                </td>

                                <td data-ordering="false">{{ $order->created_at }}</td>
                                <td data-ordering="false">{{ $order->updated_at }}</td>
                                <td>
                                    {{-- <div class="dropdown d-inline-block">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end">
                                            <li><a target="_blank" href="{{ route('admin.orders.export_order', $order) }}"
                                                    class="dropdown-item"><i
                                                        class="ri-file-list-fill align-bottom me-2 text-muted"></i>Xuất
                                                    hóa đơn</a>
                                            </li>
                                            <li><a href="{{ route('admin.orders.show', $order) }}" class="dropdown-item"><i
                                                        class="ri-eye-fill align-bottom me-2 text-muted"></i> View</a>
                                            </li>
                                            <li><a href="{{ route('admin.products.edit', $order) }}"
                                                    class="dropdown-item edit-item-btn"><i
                                                        class="ri-pencil-fill align-bottom me-2 text-muted"></i>
                                                    Edit</a></li>
                                            <li>
                                                <button data-id={{ $order->id }}
                                                    class="btn_delete dropdown-item remove-item-btn" data-bs-toggle="modal"
        ...
        <div class="col-lg-12">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <table id="table_2" class="table table-bordered dt-responsive nowrap table-striped align-middle"
                        style="width:100%">
                        <thead>
                            <tr>
                                <th scope="col" style="width: 10px;">
                                ...
                                <th data-ordering="false">Price regular</th>
                                <th data-ordering="false">Price sale</th>
                        ...
                        <tbody>
                            @foreach ($order->order_items as $item)
                                <tr>
                                    ...
                                    <td data-ordering="false"> <img width="100px"
                                            src="{{ Storage::url($item->img_thumbnail) }}" alt=""></td>
                                    <td>{{ number_format($item->price_regular) }}</td>
                                    <td data-ordering="false">{{ number_format($item->price_sale) }}</td>
                                    <td data-ordering="false">{{ $item->variant_size_name }}</td>
                                    <td data-ordering="false">{{ $item->variant_color_name }}</td>
                                    ...
                                    <td>
                                        {{-- <div class="dropdown d-inline-block">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end">
                                                ...
                                                <li><a href="{{ route('admin.orders.show', $order) }}"
                                                        class="dropdown-item"><i
                                                            class="ri-eye-fill align-bottom me-2 text-muted"></i> View</a>
                                                </li>
                                                <li><a href="{{ route('admin.products.edit', $order) }}"
