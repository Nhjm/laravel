                                    ...
                                    <div class="mt-3">
                                        ...
                                        <div class="card-body" style="width: 100%;">
                                            ...
                                            <img id="img_thumbnail_old" style="margin-top: 20px; border-radius: 10px"
                                                width="100%" src="{{ Storage::url($product->image) }}" alt="">
                                        </div>
                                        <!-- end card body -->
                            ...
                            <div class="card-body">
                                <table class="table">
                                    ...
                                    @php($rowspan = false) --}}
                                                ...
                                                <td class="" style="vertical-align: middle;"><input type="number"
                                                        value="{{ $variant->quantity }}"
                                                        name="product_variants[{{ $variant->product_size_id . '-' . $variant->product_color_id }}][quantity]"
                                                        id="" class="form-control">
                                                </td>
                                                <td class="p-4 image_variants" style="width: 300px;"
                                                    data-name="product_variants[{{ $variant->product_size_id . '-' . $variant->product_color_id }}][image]">
                                                    <div class="card-body image" style="width: 100%;">
                                                        <input type="file" class="filepond filepond-input-multiple"
                                                            multiple data-allow-reorder="true" data-max-file-size="3MB"
                                                            data-max-files="1">
                                                    </div>
                                                    <img class="mt-3" width="100%" style="border-radius: 10px"
                                        ...
                                        <div class="d-flex p-2">
                                            <div class="flex-shrink-0 me-3">
                                                ...
                                                <div class=" bg-light rounded"
                                                    style="overflow: hidden; width: 150px; max-height: 200px">
                                                    <img data-dz-thumbnail class="img-fluid rounded d-block w-100"
                                                        src="{{ Storage::url($gallery->image) }}" alt="Dropzone-Image" />
                                            ...
                                            <div class="flex-grow-1">
                                                <div class="pt-1 ">
                                                    <span class="badge badge-label bg-danger fs-4"><i></i> Ảnh cũ
                                                    </span>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#bien_the tr').each(function() {
                ...
                var colorName = $(this).find('td:eq(1)').text().trim();
            ...
            $('#tao_bien_the').on('click', function() {
                ...
                show('Đã tạo biến thể thành công!')
                    ...
                    colors.forEach(function(color) {
                        ...
                        var bien_the_html =
                            `<tr>
                                ...
                                <td class="image_variants" style="width: 280px;" data-name="product_variants[${size.id}-${color.id}][image]">
                                    <div id="image_variants-${size.id}-${color.id}" class="card-body image" style="width: 200px;">
                                            <input type="file" class="filepond filepond-input-multiple" multiple
                                                data-allow-reorder="true" data-max-file-size="3MB" data-max-files="1">
                                    </div>
                                </td>
                        ...
                        FilePond.create(document.querySelector(
                            `#image_variants-${size.id}-${color.id}`), {
                            // Cấu hình FilePond tại đây nếu cần
                            // server: '/upload', // Đường dẫn xử lý upload, cần thiết lập ở server
                            maxFileSize: '3MB', // Kích thước tối đa của tệp
                            acceptedFileTypes: ['image/*'], // Chỉ chấp nhận tệp ảnh
                            labelIdle: 'Kéo & thả file hoặc <span class="filepond--label-action">Duyệt</span>',
                            // Các tùy chọn khác...
                        });
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelector("#form_update").addEventListener("submit", function(e) {
                ...
                if (filePondFiles.length > 0) {
                    formData.append('image', filePondFiles[0].file);
                }

                let imageVariants = document.querySelectorAll('.image_variants');

                imageVariants.forEach((element) => {

                    let file_pond = FilePond.find(element.querySelector('div .image'));

                    if (file_pond) {
                        // Lấy danh sách các tệp từ FilePond instance
                        let files = file_pond.getFiles();
                        // Thêm các tệp vào FormData
                        let dataName = element.dataset.name;

                        if (files.length > 0) {
                            formData.append(`${dataName}`, files[0].file);
                        }
                    }
                })

                // Gửi formData qua fetch
                let url = '{{ route('admin.products.update', $product) }}'
            ...
            imageVariants.forEach(element => {
                ...
                let file_Pond = FilePond.create(element.querySelector('div .image'))

                file_Pond.on('addfile', (error, file) => {
                    // Nếu không có lỗi và file đã được thêm
                    if (!error) {
                        // element.querySelector('img').style.display = 'none';
                        $(element).find('img').slideUp();
                    } else {
                        console.error('Lỗi khi thêm file vào FilePond:', error);
                    }
                });
                ...
                // Lắng nghe sự kiện khi xóa file
                file_Pond.on('removefile', (error, file) => {
                    // Kiểm tra số lượng file trong FilePond
                    const files = file_Pond.getFiles();
                    if (files.length === 0) {
                        // Hiển thị lại div uploadInstruction
                        $(element).find('img').slideDown()
                    }
                });
