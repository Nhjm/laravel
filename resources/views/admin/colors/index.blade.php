                        ...
                        <div id="div_form_create" style="display: none">
                            <form id="form_create">
                                ...
                                <div class="row ">
                                    ...
                                    <div class="col-md-6 d-flex align-items-md-center ps-md-4 mb-3 align-content-center">
                                        <label class="me-3 mt-2" for="firstnamefloatingInput">Color:</label>
                                ...
                                <tbody class="list form-check-all">
                                    @foreach ($data as $item)
                                        <tr>
                                            ...
                                            <td>
                                                {{-- <div class="rounded-circle" style="width: 40px; height: 40px; ;background: {{ $item->color }}"></div> --}}
                                                <div class="color-picker-container"
                                                    data-default-color="{{ $item->color }}"
                                                    data-id_color="{{ $item->id }}">
                                                </div>
                                            </td>
...
@section('style_libs')
    ...
    <link rel="stylesheet"
        href="{{ asset('theme/admins/velzon/assets/libs/@simonwep/pickr/themes/classic.min.css') }}" />
    <!-- 'classic' theme -->
    <link rel="stylesheet"
        href="{{ asset('theme/admins/velzon/assets/libs/@simonwep/pickr/themes/monolith.min.css') }}" />
    <!-- 'monolith' theme -->
    <link rel="stylesheet" href="{{ asset('theme/admins/velzon/assets/libs/@simonwep/pickr/themes/nano.min.css') }}" />
...
@section('script_libs')
    ...
    <script src="{{ asset('theme/admins/velzon/assets/libs/@simonwep/pickr/pickr.min.js') }}"></script>
    <!-- init js -->
    <script src="{{ asset('theme/admins/velzon/assets/js/pages/form-pickers.init.js') }}"></script>
    <script>
        document.addEventListener('DOMContentLoaded', function() {

            const pickr = Pickr.create({
                el: '#color-picker',
                theme: 'nano', // hoặc 'monolith', hoặc 'nano'
                default: '#ff0000',
                appClass: 'color_input',
                components: {
                    preview: true,
                    opacity: true,
                    hue: true,
            ...
            removeNotificationModal.addEventListener('show.bs.modal', event => {
                ...
                document.getElementById('delete-notification').addEventListener('click', () => {
                    ...
                    let url = `{{ route('admin.colors.destroy', '') }}/${id}`;
            ...
            document.querySelectorAll('.name').forEach(element => {
                ...
                element.addEventListener('dblclick', () => {
                    ...
                    function use_fetch(input) {
                        let url = `{{ route('admin.colors.update', '') }}/${id}`;
            ...
            document.getElementById('submit_form_create').addEventListener('click', () => {
                ...
                form_data.append('color', colorValue)
            ...
            const pickrs = Array.from(document.querySelectorAll('.color-picker-container')).map(container => {
                const defaultColor = container.getAttribute('data-default-color');
                ...
                return Pickr.create({
                    ...
                    dataColorId: container.getAttribute('data-id_color')
                });
            });
            ...
            pickrs.forEach(pickr => {
                pickr.on('save', (color, instance) => {
                    ...
                    let color_value = color.toHEXA().toString();

                    let id = pickr.options.dataColorId
                    let url = `{{ route('admin.colors.update', '') }}/${id}`;
